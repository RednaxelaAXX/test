...
_multi_arch() {
	...
	#
	[[ ! -f "${CROSS_HOST_ARCH}-cross.tgz" ]] && curl "https://musl.cc/${CROSS_HOST_ARCH}-cross.tgz" > "${CROSS_HOST_ARCH}-cross.tgz"
	tar xf "${CROSS_HOST_ARCH}-cross.tgz" --strip-components=1 -C "${qb_install_dir}"
	#
